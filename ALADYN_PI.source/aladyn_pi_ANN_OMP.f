...
!
      ...
      double precision :: Rc_ann,d_ann,d4_ann,Gauss_ann,range_min_ann
      double precision :: ActFunc_shift
     ...
     1  U3_of
      ...
      double precision, dimension(:), allocatable :: U0,U1,U2,U3
      ...
      double precision, dimension(:), allocatable :: r0Rc,r0pRc
      double precision, dimension(:), allocatable :: U0f1,U1f1,U2f1
      double precision, dimension(:), allocatable :: U0f2,U1f2,U2f2
      double precision, dimension(:), allocatable :: U0f3,U1f3,U2f3
...
!
      ...
      double precision, intent(in) :: Rcn
  
      double precision :: dgij1,dgij2,dgij3,U_x1,U_x2,U_x3
      double precision :: Gi_sum_01,Gi_sum_11,Gi_sum_21,Gi_sum_31,
     1 Gi_sum_41
      double precision :: Gi_sum_02,Gi_sum_12,Gi_sum_22,Gi_sum_32,
     1 Gi_sum_42
      double precision :: Gi_sum_03,Gi_sum_13,Gi_sum_23,Gi_sum_33,
     1 Gi_sum_43
...
!$OMP& SCHEDULE(STATIC)
     ...
     = (r0Rc(n_set) + rij*(rij-r0pRc(n_set)) - Sigma2)*d4 - RijRo*RcRij5
         dfsij = 2.d0*dfsij*RcRij3*expRijRo / (r0*denominator)
...
!$OMP simd simdlen(8)
     ...
     +    xr_ij2(nb1,ni)*xr_ij2(nb2,ni) + xr_ij3(nb1,ni)*xr_ij3(nb2,ni)
          ...
          Pl6 =((14.4375d0*cos2_ijk - 19.6875d0)*cos2_ijk + 6.5625)*
     &         cos2_ijk - 0.3125d0
          dPl6=(86.625d0*cos2_ijk**2 - 78.75d0*cos2_ijk + 13.125d0)*
     &          cos_ijk
...
!$OMP end simd
        ...
        Gi_fact0 = 1.d0/sqrt(Gi_dev(icol,ni)**2 + 1.d0)
        Gi_fact1 = 1.d0/sqrt(Gi_dev(icol+1,ni)**2 + 1.d0)
        Gi_fact2 = 1.d0/sqrt(Gi_dev(icol+2,ni)**2 + 1.d0)
        Gi_fact3 = 1.d0/sqrt(Gi_dev(icol+3,ni)**2 + 1.d0)
        Gi_fact4 = 1.d0/sqrt(Gi_dev(icol+4,ni)**2 + 1.d0)
...
! -- Input Layer ---
       do iraw=1,Nodes_of_layer(2)
        ...
        do icol=1,Nodes_of_layer(1)
         U_vect = U_vect + 
     + log(Gi_dev(icol,ni)+sqrt(Gi_dev(icol,ni)**2+1.d0))*
     & W1_ann(icol,iraw)
        enddo
        TrFunc = 1.d0 / (1.d0 + exp(-U_vect))
        U2(iraw) = TrFunc + ActFunc_shift
        dfuN_dev(iraw,1,ni) = TrFunc*(1.d0-TrFunc)
       enddo
...
!       U1(1:NLcurr)=U2(1:NLcurr)
        ...
        do iraw=1,NLnext
         ...
         TrFunc = 1.d0 / (1.d0 + exp(-U_vect))
         U2(iraw) = TrFunc + ActFunc_shift
         dfuN_dev(iraw,layer,ni) = TrFunc*(1.d0-TrFunc)
        enddo
...
!$OMP& SCHEDULE(STATIC)
      ...
      do ni = 1,natoms   ! loop over atoms !
       ...
       Ep = U3_of(1,ni)
       Ep_of(ni) = 2.d0*Ep  ! Twice the atomic energy  !
