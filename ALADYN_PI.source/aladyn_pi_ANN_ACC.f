...
!
      ...
      double precision, dimension(:,:), allocatable :: r_ij_dev
      double precision, dimension(:,:), allocatable :: x_ij_dev
      double precision, dimension(:,:), allocatable :: y_ij_dev
      double precision, dimension(:,:), allocatable :: z_ij_dev
      double precision, dimension(:,:), allocatable :: U1f,U2f,Gi_acc
      double precision, dimension(:,:,:), allocatable :: 
     1 fsij_acc, dfuN_acc
      double precision, dimension(:,:,:), allocatable :: dfs_dxij
      double precision, dimension(:,:,:), allocatable :: dfs_dyij
      double precision, dimension(:,:,:), allocatable :: dfs_dzij
      double precision, dimension(:,:,:,:), allocatable :: Gi_3D_acc
...
!$ACC LOOP SEQ
        do nb2 = 1,maxn_nbrs     ! Loop I: i - j bond !
         ...
         fsik = 2.d0*fsij_acc(nb2,n_set,ni)
     ...
     -    - 0.3125d0
      ...
      Gi_sum4_z = Gi_sum4_z + fsik*dgij(3)
        ...
        Gi_fact0 = 1.d0/sqrt(Gi_acc(icol,ni)**2 + 1.d0)
        Gi_fact1 = 1.d0/sqrt(Gi_acc(icol+1,ni)**2 + 1.d0)
        Gi_fact2 = 1.d0/sqrt(Gi_acc(icol+2,ni)**2 + 1.d0)
        Gi_fact3 = 1.d0/sqrt(Gi_acc(icol+3,ni)**2 + 1.d0)
        Gi_fact4 = 1.d0/sqrt(Gi_acc(icol+4,ni)**2 + 1.d0)
...
!$ACC LOOP SEQ
     ...
     +   sqrt(Gi_acc(icol,ni)**2+1.d0))*W1_ann(icol,iraw)
        enddo
        TrFunc = 1.d0 / (1.d0 + exp(-U_vect))
        U2(iraw) = TrFunc + ActFunc_shift
        dfuN_acc(iraw,1,ni) = TrFunc*(1.d0-TrFunc)
       enddo
...
!$ACC LOOP SEQ
         ...
         TrFunc = 1.d0 / (1.d0 + exp(-U_vect))
         U2(iraw) = TrFunc + ActFunc_shift
         dfuN_acc(iraw,layer,ni) = TrFunc*(1.d0-TrFunc)
        enddo
